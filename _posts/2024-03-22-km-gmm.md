...
Note that in the above, the $\mu_j$ and $\pi_j$ are the parameters that we are evaluating with the function $Q$. The latest parameters $\theta^{(t)}$ are used to compute the $\gamma(z_{ij})$ by participating in the construction of the distribution from which we are sampling the latent variables.
